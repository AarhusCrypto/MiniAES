...
static Ce find_free_slot() {
  ...
  // check if COO has been initialized
  if (!cur) {
    return 0;
  }
...
bool COO_setup(OE oe, int cfnt) {
  ...
  // setup up coov4 mem 
  coov4_mem = coo_internal_special_memory(sizeof(*coov4_mem)*cfnt);
...
void COO_end() {
  Mutex_lock( coov4_lock );
  free(coov4_mem);coov4_mem = 0;
  coov4_fkt_max = 0;
  Mutex_unlock( coov4_lock );
  Mutex_destroy( coov4_lock );
}
